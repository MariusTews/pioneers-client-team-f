...
public class LobbyController implements Controller {
    ...
    private void handleTabSwitching(Tab oldValue, Tab newValue) {
        ...
        for (DirectChatStorage directChatStorage : directChatStorages) {
            if (directChatStorage.getTab() != null && directChatStorage.getTab().equals(newValue)) {
                ...
                tabDisposable = eventListener.listen("groups." + directChatStorage.getGroupId() + ".messages.*.*", Message.class)
                        .observeOn(FX_SCHEDULER).subscribe(messageEvent -> {
    ...
    public void rulesButtonPressed() {
        app.show(rulesScreenController.get());
    }

    public void logoutButtonPressed() {
        lobbyComService.logout(loginController);
    }
    ...
    public void onRejoin() {
        lobbyComService.onJoin(members, gameScreenController, userStorage);
    }
