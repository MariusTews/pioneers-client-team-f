...
public class MemberServiceTest {
	...
	void getAllGameMembers() {
		when(gameMembersApiService.findAll(any())).thenReturn(Observable.just(new ArrayList<>(Collections
				.singleton(new Member("0:30", "15:50", "01", "02", false, "#000000", false)))));
	...
	void statusUpdate() {
		when(gameMembersApiService.patch(any(), any(), any())).thenReturn(Observable.just(new Member("0:30",
				"15:50", "01", "02", false, "#000000", false)));
		Member member = memberService.statusUpdate("testGame", "testUser", false, "#000000", false).blockingFirst();
		assertEquals("Member[createdAt=0:30, updatedAt=15:50, gameId=01, userId=02, ready=false, color=#000000, spectator=false]", member.toString());
